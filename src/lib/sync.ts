...
export function initSync() {
  ...
  if (typeof document !== "undefined") {
    ...
    // setDoc 도 완료되지 못하고 변경이 영구 유실될 수 있다(서버 미반영, 마커만 잔존).
    // "백그라운드에서 못 보내면 다시 열 때 보낸다" — 다시 보일 때 PENDING_TS 마커가
    // 남아 있으면 즉시 push 한다.
    const flushOrRetry = () => {
      if (pushTimer) {
        clearTimeout(pushTimer);
